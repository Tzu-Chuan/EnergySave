...
public partial class handler_ExportTotalBehindEx : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (strStage != "")
        {
            ...
            string tmpType = "", tmpCity = "",tmpYear="", tmpSeason = "";
            ...
            if (ds.Tables[1].Rows.Count > 0)
            {
                ...
                for (int i = 0; i < ds.Tables[1].Rows.Count; i++)
                {
                    ...
                    if (tmpCity != "" && ds.Tables[1].Rows[i]["city_I_Guid"].ToString().Trim() == tmpCity && tmpYear== ds.Tables[1].Rows[i]["city_Year"].ToString().Trim() && tmpSeason == ds.Tables[1].Rows[i]["city_Season"].ToString().Trim())
                    {
                        nowColumn = 2;
                        tmpCity = ds.Tables[1].Rows[i]["RM_ProjectGuid"].ToString().Trim();
                        ...
                        tmpSeason = ds.Tables[1].Rows[i]["city_Season"].ToString().Trim();
                        u_row = u_sheet.CreateRow(nowRow);// 在工作表裡面，產生一列。
                        u_row.CreateCell(0).SetCellValue(ds.Tables[1].Rows[i]["city_Item_cn"].ToString().Trim());
                        u_row.CreateCell(1).SetCellValue(ds.Tables[1].Rows[i]["city_Year"].ToString().Trim() + "年第" + ds.Tables[1].Rows[i]["city_Season"].ToString().Trim() + "季");
                        for (int n = 0; n < ds.Tables[0].Rows.Count; n++)
                        {
                            string strkey1 = String.Format("sum_{0}_1", ds.Tables[0].Rows[n]["C_Item"].ToString().Trim());
                            string strkey2 = String.Format("sum_{0}_2", ds.Tables[0].Rows[n]["C_Item"].ToString().Trim());
            ...
            workbook.Write(ms);
            string fileName = "擴大補助-各縣市申請數第" + strStage + "期" + DateTime.Now.ToString("yyyyMMddHHmmss");
